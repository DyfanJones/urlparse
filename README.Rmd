...
`urlparse` is influenced by python's `from urllib.parse import urlparse` and doesn't split out username or password from the host.
...
This allows `urlparse` to handle fairly complex url's without sacrificing any performance.
...
Note: it is faster to use `url_modify` rather than then piping the `set_*` functions.  This is because `urlparse` has to parse the url within each `set_*` to modify the url.
